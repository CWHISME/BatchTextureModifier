...
namespace BatchTextureModifier
{
    ...
    public enum EScaleMode
    {
        ...
        StretchScale,
        [Description("ScaleBased(比例缩放)：将原图缩放至指定分辨率时尽量保持比例不变，多余处填充透明度")]
        ScaleBased,
        [Description("ScaleBasedByCut(比例裁剪)：以高度或宽度最大者为基准进行缩放，尽量保持比例不变，多余处直接裁剪")]
        ScaleBasedByCut,
        [Description("Fill(填充缩放)：如果图片小于设定分辨率，则不改变图片原有像素大小，不足之处以透明度填充；如果图片大于设定分辨率，则比例缩放")]
        Fill,
        [Description("DirectCut(直接裁剪)：直接裁剪至目标分辨率")]
        DirectCut,
        [Description("WidthBase(基于宽度)：以宽度为基准进行缩放，高度不足则以透明度填充，高度超过之处则裁剪")]
        WidthBase,
        [Description("HeightBase(基于高度)：以高度为基准进行缩放，宽度不足则以透明度填充，宽度超过之处则裁剪")]
        HeightBase,
